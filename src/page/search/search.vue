<template>
  <div class="paddingTop search_page">
     ...
     <section class="search_history" v-if="searchHistory.length&&showHistory">
         ...
         <ul>
             <li v-for="(item,index) in searchHistory" :key="index" class="history_list" @click="deleteHistory">
...
export default {
  ...
  methods:{
      ...
      async searchTarget(historyValue){
        ...
        //点击搜索结果 进入下一页时进行判断是否有一样的历史记录 没有就新增 有就不做重复储存 判断完成后进入下一页
      ...
      //搜索结束后 删除搜索内容为空时 显示历史记录
      checkInput(){
          if(this.searchValue == ''){
              this.showHistory = true;
              this.emptyResult = false;//隐藏搜索结果为空的提示
