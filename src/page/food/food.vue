  ...
  <div class="food_container">
    ...
    <section class="sort_container">
          ...
          <section v-show="sortBy == 'food'" class="category_container sort_detail_type">
            ...
            <section class="category_right">
              <ul>
                ...
                :class="{category_right_choosed:restaurant_category_ids == item.id||(!restaurant_category_ids)&&index == 0}" @click="getCategoryIds(item.id,item.name)">
                  <span>{{item.name}}</span>
      ...
      <div class="sort_item" :class="{choose_type:sortBy == 'sort'}">
       ...
       <!-- 筛选 -->
       <div class="sort_item" :class="{chooes_type:sortBy == 'activity'}">
             ...
             <section style="width:100%">
               ...
               <ul class="filter_ul">
                 <li v-for="(item,index) in Delivery" :key="index" class="filter_li" @click="selectDeliveryMode(item.id)">
                   ...
                   <span :class="{secected_filter:delivery_mode == item.id}">{{item.text}}</span>
                 </li>
               </ul>
             </section>
             <section style="width:100%">
         ...
         <section class="shop_list_container">
          	<shop-list :geohash="geohash" :restaurantCategoryId="restaurant_category_id" :restaurantCategoryIds="restaurant_category_ids" 
            :sortByType='sortByType' :deliveryMode="delivery_mode" :confirmSelect="confirmStatus" :supportIds="support_ids" v-if="latitude">
            </shop-list>
         </section>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async initData(){
       ...
       //初始化定位当前category分类左侧默认选择项，在右侧展示出其sub_categories列表
    ...
    //选择category右侧的选项时 进行筛选 重新获取数据并渲染
    ...
    selectCategoryName(id,index){
       //第一个选项 --全部商家 因为没有自己的列表 所以右侧点击默认获取所有数据
       if(index === 0){
          this.restaurant_category_ids = null;
          this.sortBy = "";
          //不是第一个选项时 右侧展示其子集sub_categories的列表
       }else{
    ...
    selectDeliveryMode(id){
       //delivery_mode为空时 选中当前项 并且filterNum++
       if(this.delivery_mode == null){
          this.filterNum++;
          this.delivery_mode = id;
          //delivery_mode为当前已有值时，清空所选项 并且filterNum--
