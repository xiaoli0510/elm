 <template>
  <div class="rating_page">
      <section v-if="!showLoading">
         ...
         <section class="quick_remark" v-if="remarkList.remarks">
             ...
             <ul class="remark_arr_list_ul">
                 <li class="remark_arr_list_li" v-for="(item,index) in remarkList.remarks" :key="index">
                     <span :class="{first:remarkIndex == 0,last:remarkIndex == item.length-1,choosed:remarkText[index]&&(remarkText[index][0] == remarkIndex)}" class="remark_item_li"
                      v-for="(remarkItem,remarkIndex) in item" :key="remarkIndex" @click="chooseRemark(index,remarkIndex,remarkItem)">
                       {{remarkItem}}
                     </span>
                 </li>
             </ul>
         ...
         <section class="input_remark quick_remark">
             ...
             <textarea class="input_text" v-model="inputText" placeholder="请输入 备注内容(可不填)"></textarea>
         </section>
